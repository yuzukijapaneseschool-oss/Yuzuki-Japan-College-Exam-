...
async function sendAdmissionCardEmail(studentData) {
  ...
  try {
    ...
    const adminMailOptions = {
      ...
      to: COLLEGE_ADMIN_EMAILS.join(', '),
      replyTo: (email && email.includes('@')) ? email : 'yuzukijapaneseschool@gmail.com',
      subject: '🎓 [New Admission] ' + student_id + ' - ' + name + ' (' + (course_name || 'Course') + ')',
      html: adminHtml
    };

    const adminInfo = await transporter.sendMail(adminMailOptions);
    console.log('✉️ Admin notification dispatched to ' + COLLEGE_ADMIN_EMAILS.join(', ') + ' for ' + student_id + '. MessageId: ' + adminInfo.messageId);
  ...
  const primaryAdmin = 'yuzukijapaneseschool@gmail.com';
  if (email && email.includes('@') && !COLLEGE_ADMIN_EMAILS.includes(email.toLowerCase())) {
    try {
...
module.exports = {
  ...
  COLLEGE_ADMIN_EMAILS,
  COLLEGE_ADMIN_EMAIL: 'yuzukijapaneseschool@gmail.com'
};

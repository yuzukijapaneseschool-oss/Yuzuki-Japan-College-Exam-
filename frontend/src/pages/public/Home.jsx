...
// Real JFT Pass Achievers (All 11 Official Results)
import passDinithi from '../../assets/jft_pass_dinithi.jpg';
import passThanuja from '../../assets/jft_pass_thanuja.jpg';
import passDilmi from '../../assets/jft_pass_dilmi.jpg';
import passPraveen from '../../assets/jft_pass_praveen.jpg';
import passKavindu from '../../assets/jft_pass_kavindu.jpg';
import passTharindu from '../../assets/jft_pass_tharindu.jpg';
import passMatheesh from '../../assets/jft_pass_matheesh.jpg';
import passSachini from '../../assets/jft_pass_sachini.jpg';
import passVishal from '../../assets/jft_pass_vishal.jpg';
import passSithum from '../../assets/jft_pass_sithum.jpg';
import passKaushika from '../../assets/jft_pass_kaushika.jpg';
...
export default function Home() {
  ...
  const achieverSliderRef = useRef(null);
  const [canScrollLeft, setCanScrollLeft] = useState(false);
  const [canScrollRight, setCanScrollRight] = useState(true);
  const [activeSlideIndex, setActiveSlideIndex] = useState(0);

  const checkAchieverScroll = () => {
    if (achieverSliderRef.current) {
      const { scrollLeft, scrollWidth, clientWidth } = achieverSliderRef.current;
      setCanScrollLeft(scrollLeft > 10);
      setCanScrollRight(scrollLeft < scrollWidth - clientWidth - 10);
      const cardWidth = 280 + 20; // width + gap
      const index = Math.round(scrollLeft / cardWidth);
      setActiveSlideIndex(Math.min(Math.max(index, 0), 10));
    }
  };

  const scrollAchieverLeft = () => {
    if (achieverSliderRef.current) {
      achieverSliderRef.current.scrollBy({ left: -320, behavior: 'smooth' });
    }
  };

  const scrollAchieverRight = () => {
    if (achieverSliderRef.current) {
      achieverSliderRef.current.scrollBy({ left: 320, behavior: 'smooth' });
    }
  };

  const scrollToAchieverIndex = (index) => {
    if (achieverSliderRef.current) {
      const cardWidth = 280 + 20;
      achieverSliderRef.current.scrollTo({ left: index * cardWidth, behavior: 'smooth' });
    }
  };

  useEffect(() => {
    const el = achieverSliderRef.current;
    if (!el) return;
    let isHovered = false;
  ...
  const achievers = [
    { name: 'Dinithi Uthsara', score: '232 / 250', exam: 'JFT-Basic A2.2 (Top Pass)', img: passDinithi },
    ...
    { name: 'Praveen Aththanayake', score: '219 / 250', exam: 'JFT-Basic A2.2 (Pass)', img: passPraveen },
    { name: 'Kavindu Siriwardhana', score: '215 / 250', exam: 'JFT-Basic A2.2 (Pass)', img: passKavindu },
    { name: 'Tharindu Lakshan', score: '210 / 250', exam: 'JFT-Basic A2.2 (Pass)', img: passTharindu },
    ...
    { name: 'Vishal Adithya', score: '205 / 250', exam: 'JFT-Basic A2.2 (Pass)', img: passVishal },
    { name: 'Sithum Lakmewala', score: '205 / 250', exam: 'JFT-Basic A2.2 (Pass)', img: passSithum },
    { name: 'Kaushika Praveen', score: '200 / 250', exam: 'JFT-Basic A2.2 (Pass)', img: passKaushika }
  ];
  ...
  return (
    <div className="bg-slate-50 text-slate-900 overflow-hidden font-japanese">
      ...
      {/* 🏆 Top JFT-Basic Achievers: Interactive Wall of Fame Carousel */}
      <section className="py-20 px-4 max-w-7xl mx-auto space-y-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div className="space-y-3 max-w-3xl">
            <div className="inline-flex items-center space-x-2 bg-amber-100 text-amber-900 border border-amber-300 px-4 py-1 rounded-full text-xs font-bold font-mono">
              ...
              <span>Japan Foundation Official Exam Results (11 Successful Candidates)</span>
            </div>
            <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900">
              Congratulations to Our JFT-Basic High Achievers! 🎉
            </h2>
            <p className="text-slate-600 text-sm leading-relaxed">
              Real Japan Foundation scorecards & assessment certificates from YUZUKI Japan College students passing on their very first attempt!
            </p>
          </div>
          ...
          <div className="flex items-center space-x-3 shrink-0">
            <button
              onClick={scrollAchieverLeft}
              disabled={!canScrollLeft}
              className={`p-3 rounded-2xl border transition-all flex items-center justify-center ${
                canScrollLeft 
                  ? 'bg-white hover:bg-rose-600 text-slate-800 hover:text-white border-slate-300 shadow-md hover:scale-105 active:scale-95' 
            ...
            <button
              onClick={scrollAchieverRight}
              disabled={!canScrollRight}
              className={`p-3 rounded-2xl border transition-all flex items-center justify-center ${
                canScrollRight 
                  ? 'bg-rose-600 hover:bg-rose-700 text-white border-rose-600 shadow-lg shadow-rose-600/30 hover:scale-105 active:scale-95' 
                  : 'bg-slate-100 text-slate-400 border-slate-200 cursor-not-allowed opacity-60'
              }`}
              title="Next Achiever"
            >
        ...
        <div className="relative group">
          ...
          <button
            onClick={scrollAchieverLeft}
            className="hidden lg:flex absolute -left-5 top-1/2 -translate-y-1/2 z-20 w-12 h-12 bg-white/95 hover:bg-rose-600 text-slate-800 hover:text-white rounded-full shadow-2xl border border-slate-200 hover:border-rose-600 items-center justify-center transition-all transform hover:scale-110 active:scale-95 backdrop-blur-md opacity-90 group-hover:opacity-100"
          ...
          <div
            ref={achieverSliderRef}
            onScroll={checkAchieverScroll}
            className="flex items-stretch gap-5 overflow-x-auto scroll-smooth snap-x snap-mandatory py-4 px-1 no-scrollbar"
          ...
          >
            {achievers.map((ach, idx) => (
              <div 
                ...
                className="flex-none w-[270px] sm:w-[290px] snap-start bg-white rounded-3xl border-2 border-slate-200 hover:border-amber-400 overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 cursor-pointer group/card flex flex-col justify-between transform hover:-translate-y-2"
              >
                ...
                <div className="h-72 overflow-hidden relative bg-slate-900">
                  ...
                  {/* Top Badge */}
                  <div className="absolute top-3 right-3 bg-slate-950/85 backdrop-blur-md text-amber-300 text-[10px] font-black px-2.5 py-1 rounded-full border border-amber-400/40 shadow-lg">
                    {ach.exam}
                  </div>

                  {/* Hover Overlay */}
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-950/90 via-slate-950/30 to-transparent opacity-0 group-hover/card:opacity-100 transition-opacity flex items-end p-4 justify-center">
                    <span className="bg-white text-slate-900 text-xs font-bold px-4 py-2 rounded-2xl shadow-xl flex items-center space-x-1.5 transform translate-y-2 group-hover/card:translate-y-0 transition-transform">
                ...
                <div className="p-4 bg-white border-t border-slate-100 space-y-2">
                  <div className="flex items-center space-x-1.5 text-[11px] text-amber-600 font-bold">
                    ...
                    <span className="truncate">Japan Foundation Test Pass</span>
                  </div>

                  <h4 className="font-extrabold text-base text-slate-900 group-hover/card:text-rose-600 transition-colors truncate">
                    {ach.name}
                  </h4>

                  <div className="flex items-center justify-between text-xs pt-2 border-t border-slate-100">
                    <div>
                      <span className="text-slate-400 text-[10px] block uppercase font-mono">Exam Score</span>
                      <span className="text-emerald-600 font-black text-sm font-mono">{ach.score}</span>
                    </div>
                    <span className="bg-emerald-100 text-emerald-800 text-[11px] font-black px-2.5 py-1 rounded-xl border border-emerald-200 flex items-center space-x-1">
          ...
          <button
            onClick={scrollAchieverRight}
            className="hidden lg:flex absolute -right-5 top-1/2 -translate-y-1/2 z-20 w-12 h-12 bg-white/95 hover:bg-rose-600 text-slate-800 hover:text-white rounded-full shadow-2xl border border-slate-200 hover:border-rose-600 items-center justify-center transition-all transform hover:scale-110 active:scale-95 backdrop-blur-md opacity-90 group-hover:opacity-100"
        ...
        <div className="flex items-center justify-center space-x-2 pt-2">
          {achievers.map((_, i) => (
            <button
              key={i}
              onClick={() => scrollToAchieverIndex(i)}
              className={`h-2.5 rounded-full transition-all ${
                activeSlideIndex === i ? 'w-8 bg-rose-600' : 'w-2.5 bg-slate-300 hover:bg-slate-400'
              }`}

...
export default function ExamSession() {
  ...
  const isJft = Boolean(exam?.is_jft || exam?.course_code === 'JFT-BASIC' || (questions.length === 60 && exam?.duration_minutes > 0));
  ...
  const sectionMinIndex = isJft ? (currentSectionNum - 1) * 15 : 0;
  const sectionMaxIndex = isJft ? Math.min(questions.length - 1, currentSectionNum * 15 - 1) : questions.length - 1;

  const handleNextInCurrentSection = () => {
    if (currentIndex < sectionMaxIndex) {
      setCurrentIndex(prev => prev + 1);
    } else {
      if (isJft && currentSectionNum < 4 && questions.length > currentSectionNum * 15) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
          ...
          <div className="lg:col-span-3 bg-white rounded-2xl border border-slate-200/80 shadow-md p-6 sm:p-8 space-y-6">
            ...
            <div className="pt-6 border-t border-slate-100 flex items-center justify-between">
              ...
              ) : isJft && currentSectionNum < 4 && questions.length > currentSectionNum * 15 ? (
                <button
                  type="button"
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-slate-900 text-sm font-japanese">
                  {isJft ? '問題パレット (15問単位)' : '問題一覧 (Question Palette)'}
                </h3>
                <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${
                  isJft ? 'text-rose-700 bg-rose-50 border border-rose-200' : 'text-emerald-700 bg-emerald-50 border border-emerald-200'
                }`}>
                  {isJft ? `Sec ${currentSectionNum} 進行中` : `All ${questions.length} Questions`}
                </span>
              </div>
                ...
                <div className="p-3 rounded-2xl border bg-slate-50 border-slate-200">
                  <div className="grid grid-cols-5 gap-1.5 max-h-[380px] overflow-y-auto p-1">
                    {questions.map((q, globalIdx) => {
                      ...
                      if (isAnswered) btnStyle = "bg-emerald-600 text-white border-emerald-700 hover:bg-emerald-700";
                      if (isFlag) btnStyle = "bg-amber-500 text-white border-amber-600 hover:bg-amber-600";
                      ...
                      return (
                        <button
                          ...
                          onClick={() => handlePaletteClick(globalIdx)}
                          className={`h-8 rounded-lg font-mono text-xs font-bold border transition-all flex items-center justify-center relative ${btnStyle} ${
                            isCurrent ? 'ring-2 ring-emerald-500 ring-offset-1 scale-105 shadow font-black' : ''
                          }`}
                          title={`Question ${globalIdx + 1}`}
                        >
                          {globalIdx + 1}
                          {isFlag && <span className="w-1.5 h-1.5 rounded-full bg-white absolute top-0.5 right-0.5" />}
                        </button>
                      );
                    })}
                  </div>
